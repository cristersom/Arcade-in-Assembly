...
.enemy_reset_skip:
    ...
    add word ptr [score], 10    ; Adiciona 10 pontos ao score
    call atualiza_score_hud     ; Atualiza o HUD com o novo score
...
.tiro_update_skip:
    ...
    ;-----------------------------------------------------------
    ; --- IN?CIO DA SE??O DE COLIS?O ---
    ;-----------------------------------------------------------
    
    ; --- FIX PARA "JUMP OUT OF RANGE" ---
    cmp tiro_ativo, 1
    je .faz_chegagem_colisao_tiro ; Se tiro EST? ativo, pula para o c?digo de checagem
    jmp .pula_colisao_tiro        ; Se tiro N?O est? ativo, pula o bloco todo (JMP longo)

.faz_chegagem_colisao_tiro:
    ; --- Checa colis?o: Tiro vs Inimigo 1 ---
    push word ptr [tiro_x]   ; x1
    push word ptr [tiro_y]   ; y1
    push word ptr [TIRO_W]   ; w1
    push word ptr [TIRO_H]   ; h1
    push word ptr [enemy1_x] ; x2
    push word ptr [enemy1_y] ; y2
    push word ptr [SPRITE_W] ; w2
    push word ptr [SPRITE_H] ; h2
    call checa_colisao
    
    cmp ax, 1 ; AX=1 significa que colidiu
    jne .pula_colisao_inimigo1

    ; COLIDIU COM INIMIGO 1!
    mov tiro_ativo, 0           ; Desativa o tiro
    add word ptr [score], 100   ; Adiciona 100 pontos [Source: 104]
    call atualiza_score_hud     ; Atualiza o placar na tela
    mov word ptr [enemy1_x], 400 ; Move o inimigo para fora da tela
    jmp .pula_colisao_tiro      ; J? acertou, n?o precisa checar os outros

.pula_colisao_inimigo1:
    ; --- Checa colis?o: Tiro vs Inimigo 2 ---
    push word ptr [tiro_x]
    push word ptr [tiro_y]
    push word ptr [TIRO_W]
    push word ptr [TIRO_H]
    push word ptr [enemy2_x]
    push word ptr [enemy2_y]
    push word ptr [SPRITE_W]
    push word ptr [SPRITE_H]
    call checa_colisao
    
    cmp ax, 1
    jne .pula_colisao_inimigo2

    ; COLIDIU COM INIMIGO 2!
    mov tiro_ativo, 0
    add word ptr [score], 100
    call atualiza_score_hud
    mov word ptr [enemy2_x], 400
    jmp .pula_colisao_tiro

.pula_colisao_inimigo2:
    ; --- Checa colis?o: Tiro vs Inimigo 3 ---
    push word ptr [tiro_x]
    push word ptr [tiro_y]
    push word ptr [TIRO_W]
    push word ptr [TIRO_H]
    push word ptr [enemy3_x]
    push word ptr [enemy3_y]
    push word ptr [SPRITE_W]
    push word ptr [SPRITE_H]
    call checa_colisao
    
    cmp ax, 1
    jne .pula_colisao_tiro ; (pula para o final)

    ; COLIDIU COM INIMIGO 3!
    mov tiro_ativo, 0
    add word ptr [score], 100
    call atualiza_score_hud
    mov word ptr [enemy3_x], 400
    ; N?o precisa de jmp, j? ? o fim da checagem

.pula_colisao_tiro:
    ;-----------------------------------------------------------
    ; --- FIM DA SE??O DE COLIS?O ---
    ;-----------------------------------------------------------
...
atualiza_tempo_hud proc
    ...
    mov al,0h
    xor bh,bh
    mov bl,15 ; <-- Corrigido para Branco
    mov cx,TAM_MSG6
    mov dh,0
    mov dl,78
    int 10h
